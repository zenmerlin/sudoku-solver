...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname sudoku-v1.rkt) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
(check-expect (next-boards BD1)
              (list (fill-square BD1 (r-c->pos 0 0) 1)
                    (fill-square BD1 (r-c->pos 0 0) 2)
                    (fill-square BD1 (r-c->pos 0 0) 3)
                    (fill-square BD1 (r-c->pos 0 0) 4)
                    (fill-square BD1 (r-c->pos 0 0) 5)
                    (fill-square BD1 (r-c->pos 0 0) 6)
                    (fill-square BD1 (r-c->pos 0 0) 7)
                    (fill-square BD1 (r-c->pos 0 0) 8)
                    (fill-square BD1 (r-c->pos 0 0) 9)))
(check-expect (next-boards BD4)
              (list (fill-square BD4 (r-c->pos 0 3) 6)
                    (fill-square BD4 (r-c->pos 0 3) 8)))
...
(check-expect (find-blank BD3) 1)

#;; Initial Solution
(define (find-blank bd)
  (local [(define (fn-for-bd n bd)
            (if (false? (first bd))
                n
                (fn-for-bd (add1 n) (rest bd))))]
    (fn-for-bd 0 bd)))
...
(check-expect (fill-with-1-9 1 BD3)
              (list (fill-square BD3 1 1)
                    (fill-square BD3 1 2)
                    (fill-square BD3 1 3)
                    (fill-square BD3 1 4)
                    (fill-square BD3 1 5)
...
(define (fill-with-1-9 p bd)
  (local [(define (fn-for-lobd n p bd)
            (cond [(= n 10) empty]
                  [else (cons (fill-square bd p n)
                              (fn-for-lobd (add1 n) p bd))]))]
    (fn-for-lobd 1 p bd)))
...
(check-expect (keep-only-valid (list (cons 1 (cons 1 (rest (rest BD2))))))
              empty)
...
(define (valid-board? bd)
  (local [(define (valid-unit? u)
            (no-dup? (filter number? (read-unit u bd))))]
    (andmap valid-unit? UNITS)))


;; Unit Board -> (listof Val|false)
;; produce list of values from the given Unit of the given Board
(check-expect (read-unit (first ROWS) BD2)
              (list 1 2 3 4 5 6 7 8 9))
(check-expect (read-unit (first COLS) BD3)
              (list 1 2 3 4 5 6 7 8 9))
(check-expect (read-unit (first BOXES) BD4)
              (list 2 7 4 
                    1 B B 
                    6 B B))

(define (read-unit u bd)
  (map (λ (p) (read-square bd p)) u))


;; (listof Val) -> Boolean
;; produces true if no duplicates are found in lov
;; Assume: all lists received will contain only numbers, no blanks
(check-expect (no-dup? (list 1 2 3 4 5 6 7 8 9)) true)
(check-expect (no-dup? (list 1 2 4 7 9)) true)
(check-expect (no-dup? (list 1 2 3 4 5 6 7 5 9)) false)
(check-expect (no-dup? (list 1 9 3 4 5 6 7 5 9)) false)
